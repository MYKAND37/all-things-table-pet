...
/**
 * THIS FILE IS THE 0.18.0 SOLVER, on purpose, and this build is a test build.
 *
 * The owner remembers 0.18.0 as the version that did not shake and has asked, several times,
 * whether we should go back to it. Everything since then says no -- 0.18.0 applies the exact
 * aim every iteration with no per-frame rate cap, no lever guard and no low pass on the
 * finger, which is precisely the combination the later work measured as the buzz (39% of
 * frames reversing, 22 degrees of mean step, a 140 degree peak) -- but a number in a mirror
 * is not the same thing as a pet under a thumb, and the thumb is the only judge that counts.
 *
 * So this build exists to be FELT next to 1.9.0's. The solver below is 0.18.0's, line for
 * line; the additions are plumbing the app grew since (see the block in the class body),
 * the tuning panel's two knobs are dead on purpose, and nothing here should be merged into
 * main: the answer this build gives is "which one feels better", and that answer then
 * decides what main does.
 *
...
class Ragdoll(
    ...
    companion object {
        ...
        /**
         * Dead knobs, kept only so the app compiles against the same bench.
         *
         * The 0.18.0 solver has neither of these: it applies the whole aim every iteration
         * (no rate cap) and multiplies it by nothing (no gain). Moving either slider in this
         * build changes nothing at all, which is itself the answer to "what did 1.9.0
         * actually change".
         */
