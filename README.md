...
| | |
|---|---|
| **角色** | 19 根骨骼的素体（节点可以设「手指能不能拖」，节点显示在全局设置里开关） + 自己搭骨架（加/删/改父级/节点/属性）；一个角色可以有**多套骨骼**（几副身体，规则里切换）；整只可以**导出/导入**成 `.atppet` |
| **画画** | 每根骨头一张整画布 PNG，导入时对位；**状态换图**（穿衣服 / 机械臂那种）并且可以选 **替换**（另一张画）或 **叠加**（绷带、伤口、配件盖上去）；图层顺序可编辑，还能按状态分组调；同一节有好几套替换图时用**权重**决定哪张优先 |
...
| **液体** | 会流、会积成水洼、会绕着身体流；**液滴大小 / 半透明 / 白色**自己定，也能选**画在角色前面还是后面** |
| **粒子** | 血、汗、火花、灰尘……自己画图案、定颜色大小、受不受重力、留不留印子；每一种还能选**画在角色前面还是后面** |
